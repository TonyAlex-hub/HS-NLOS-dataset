...
def Convert_Labels(source_labels):
    ...
    if len(shape)==2:
        dst_labels = np.zeros([shape[0], shape[1], 1], dtype=np.uint8)
        for horizontal_i in range(0, shape[0]):
            for vertical_i in range(0, shape[1]):
                if source_labels[horizontal_i, vertical_i] == 0:
                    dst_labels[horizontal_i, vertical_i] = 0
                else:
                    dst_labels[horizontal_i, vertical_i] = 1
    else:
        dst_labels=np.zeros([shape[0],shape[1],shape[2],2],dtype=np.uint8)
...
def convert_3D(label_map,pixel_value1,pixel_value2):
    shape=source_labels.shape
    if len(shape)==2:
        dst_labels = np.zeros([shape[0], shape[1], 1], dtype=np.float32)
        for horizontal_i in range(0, shape[0]):
            for vertical_i in range(0, shape[1]):
                if source_labels[horizontal_i, vertical_i] == 0:
                    dst_labels[horizontal_i, vertical_i] = 1
...
if __name__ =='__main__':
    label = cv.imread("/data1/cmy/convcrf/11128871_15/original_label/11128870_1408.tif", cv.IMREAD_GRAYSCALE)
    label=Convert_Labels(label)
    cv.imwrite('/data1/cmy/convcrf/11128871_15/original_label/'+ "11128870_1408_2_chanel_map" + '.tif',label)
    #generate_name_date()
    #generate_name_time()
