...
from standard import auxiliary_1C
from standard import auxiliary
...
def merge_map_segmentation(seg,name_list,class_num=2):
    ...
    for i in range(0, len(seg)):
        ...
        if class_num==2:
            inverse =auxiliary_1C.convert_labels1C_inverse(seg[i])
        elif class_num==3:
    ...
    return images,names
def merge_map_segmentation_thr(seg,name_list,thr=0.5,class_num=2):
    seg_inverse = []
    images=[];names=[]
    for i in range(0, len(seg)):
        #将概率map转化为（0,1）矩阵
        ...
        if len(shape)==4:
            seg[i]=np.reshape(seg[i],[shape[1],shape[2], shape[3]])
        if class_num==2:
            inverse =auxiliary_1C.convert_labels1C_inverse_thr(seg[i], thr)
        elif class_num==3:
            inverse = auxiliary_3C.labels_inverse_3C(seg[i])
        seg_inverse.append(inverse)
    image_seg, name_seg = merge_image_list(seg_inverse, name_list)
    ...
    while True:
        ...
        if args.map_channel==1:
            o_patch = np.fromstring(
                o_val, dtype=np.float16).reshape((o_side, o_side, args.sat_channel))
...
def Create_args():
    ...
    parser.add_argument('--sat_size', type=int, default=24)
    parser.add_argument('--sat_channel', type=int, default=3)
    parser.add_argument('--map_size', type=int, default=8)
    parser.add_argument('--map_channel', type=int, default=1)
    parser.add_argument('--test_image_db', type=str, default=
    '/public/lpl/datasets/roads/test/roads_24_24_8_8_1/lmdb/sat')
    parser.add_argument('--test_label_db', type=str, default=
    '/public/lpl/datasets/roads/test/roads_24_24_8_8_1/lmdb/map')
    args = parser.parse_args()
    return args
'''测试函数，请勿使用'''
if __name__=='__main__':
    args=Create_args()
    batchs_data, batchs_label, key=GetData(args,args.test_image_db,args.test_label_db)
    #label= auxiliary_1C.convert_labels1C_inverse(multi_label[0])
    image_list,name_list=merge_image_list(batchs_label, key)
    path='/public/lpl/datasets/roads/test/images/'
    for i in range(0,len(image_list)):
        image,name=merge_image(image_list[i],name_list[i])
        save_image(image,path+name+'.tif')
