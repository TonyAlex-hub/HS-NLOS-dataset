...
'''计算完整影像的精确度。由于影像过大，因此需要分割后进行逐步预测。此函数根据影像块名，首先拼接影像，
然后计算单幅影像的精确度
未改动'''
...
def Accuracy_Multi(prediction,label):
    ...
    for i in range(0,shape[0]*shape[1]*shape[2]):
        ...
        if label_result[i]==1 and prediction_result[i]==1:
            TP_1=TP_1+1
        elif label_result[i]!=1 and prediction_result[i]!=1:
            TN_1=TN_1+1
        elif label_result[i]==1 and prediction_result[i]!=1:
            FN_1=FN_1+1
        elif label_result[i]!=1 and prediction_result[i]==1:
            FP_1=FP_1+1
        #目标2的精度
        if label_result[i]==2 and prediction_result[i]==2:
            TP_2=TP_2+1
        elif label_result[i]!=2 and prediction_result[i]!=2:
            TN_2=TN_2+1
        elif label_result[i]==2 and prediction_result[i]!=2:
            FN_2=FN_2+1
        elif label_result[i]!=2 and prediction_result[i]==2:
...
v2代表属于预测目标的概率。
未改动'''
def Accuracy_Multi_3C(prediction,label):
    '''According to the output(prediction) of model and label, we can
...
def Accuracy_Multi_Relax(prediction,label,N):
    ...
    if sum(label_result.flatten()) == 0:
        ...
        if sum(prediction_result.flatten()) == sum(label_result.flatten()):
            ...
            '''如果标签全是0,而预测结果有非0，那么我们认为这是预测误差，将recall_road设置为
            -1，作为一个标志'''
            error = float(sum(prediction_result.flatten())) / len(shape[0]*shape[1]*shape[2])
            error_flag = -1
...
未改动'''
def labels_inverse_3C(multi_label):
...
'''将label_map中的每个像素转化为3D向量,该函数主要用于三分类。例如：label-map:(1,2;0,1),
转化后为((0,1,0),(0,0,1);(1,0,0),(0,1,0))。其中向量中的第一元素代表是否为背景，第二个元素代表是否为目标1,
第二个元素代表是否为目标2。1表示真，0表示假。
Note:函数只适用于三类目标分类'''
def convert_3D(label_map,pixel_value1,pixel_value2):
    shape=label_map.shape
    label_map=np.reshape(label_map,[shape[0],shape[1],shape[2]])
    dst_labels=np.zeros([shape[0],shape[1],shape[2],3],dtype=np.float32)
    for batchsize_i in range(0,shape[0]):
        for horizontal_i in range(0,shape[1]):
            for vertical_i in range (0,shape[2]):
                if label_map[batchsize_i,horizontal_i,vertical_i]==pixel_value1:
                    dst_labels[batchsize_i,horizontal_i,vertical_i,1]=1
                elif label_map[batchsize_i,horizontal_i,vertical_i]==pixel_value2:
                    dst_labels[batchsize_i, horizontal_i, vertical_i, 2] = 1
                else:
