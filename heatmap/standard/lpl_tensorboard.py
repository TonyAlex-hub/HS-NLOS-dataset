...
def variable_summaries(var):
    with tf.name_scope('summaries'):
        ...
        tf.summary.scalar('mean',mean)#图片中道路占所有像素点的均值
        with tf.name_scope('stddev'):
            stddev=tf.sqrt(tf.reduce_mean(tf.square(var-mean)))
        tf.summary.scalar('stddev',stddev)
        tf.summary.scalar('max',tf.reduce_max(var))#好像没用
        tf.summary.scalar('min',tf.reduce_min(var))#好像没用
...
def gradient_summaries(gradients):
    with tf.name_scope("gradient"):
        for g, v in gradients:
            square_v=tf.square(v)#计算梯度的平方，有的梯度是一个数有的是个矩阵
            sum_v=tf.reduce_sum(square_v)#计算某一层的梯度平方和
            tf.summary.scalar(v.name,sum_v)#将名字和与之对应的梯度平方和绘成直方图。
