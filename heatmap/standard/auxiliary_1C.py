...
'''
def convert_labels1C_inverse(multi_label):
    '''Convert the predicted image(Two-channel)to normal label!'''
    shape=multi_label.shape
    label = np.zeros(shape)
    for i in range(0, shape[0]):
        for j in range(0,shape[1]):
            if multi_label[i][j]< 0.5:
                label[i][j] = 0
            elif multi_label[i][j]>= 0.5:
                label[i][j] = 255
    return label
def convert_labels1C_inverse_thr(multi_label,thr):
    '''Convert the predicted image(Two-channel)to normal label!'''
    shape=multi_label.shape
    label = np.zeros(shape)
    for i in range(0, shape[0]):
        for j in range(0, shape[1]):
            if multi_label[i][j] < thr:
                label[i][j] = 0
            elif multi_label[i][j] >= thr:
                label[i][j] = 255
    return label
'''将label-map中的每个像素转化为向量。例如：label-map:(255,0;0,255),转化后为(1,0;0,1)。1表示真，0表示假。
Note:函数只适用于单类目标预测'''
def Convert_Labels(source_labels):
   dst_labels=source_labels/255
   # shape = source_labels.shape
   # dst_labels = np.zeros(shape, dtype=np.float32)
   ...
   #                 dst_labels[batchsize_i, horizontal_i] = 1
   return dst_labels
