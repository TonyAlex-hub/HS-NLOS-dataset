...
from standard import auxiliary_3C
...
def merge_image_list(predicted_list,name_list):
    ...
    if index == len(predicted_list) - 1 and cur_name == next_name:
        image_list.append(merge)
        image_name_list.append(merge_name)
            # if (index+1)%121==0 and cur_name==next_name:
            #     image_list.append(merge)
            #     image_name_list.append(merge_name)
...
if __name__=='__main__':
    ...
    label= auxiliary.convert_labels2C_inverse(multi_label[0])
    image_list,name_list=merge_image_list(batchs_label, key)
    path='/home/test/lpl/ssai-lpl/images/'
    for i in range(0,len(image_list)):
        image,name=merge_image(image_list[i],name_list[i])
